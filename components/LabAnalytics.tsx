...
const LabAnalytics: React.FC = () => {
  ...
  const handleReportUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onloadend = async () => {
      ...
      try {
        const result = await geminiService.analyzeLabReport(base64);
        setAnalysis(result);
      } catch (err) {
      ...
      {!analysis && !loading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="bg-white p-10 rounded-[2.5rem] border border-slate-100 shadow-xl flex flex-col items-center text-center">
            ...
            <div className={`mt-10 px-6 py-2 rounded-full font-black text-xs uppercase tracking-widest border ${
              analysis.riskLevel === 'Low' ? 'bg-success/5 text-success border-success/20' : 
